...
.findAllRestaurantsCloseBy(
  ...
  // assertEquals("10", response.getRestaurants().get(0).getRestaurantId());
  // assertEquals("11", response.getRestaurants().get(1).getRestaurantId());
  // assertEquals("12", response.getRestaurants().get(2).getRestaurantId());

  ArgumentCaptor<Double> radiusCaptor =
...
verify(restaurantRepositoryServiceMock, times(1))
  .findAllRestaurantsCloseBy(
     ...
     assertEquals("5.0", radiusCaptor.getValue().toString());
  }
